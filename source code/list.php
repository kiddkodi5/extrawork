...
<body>
	...
	<table border="1px">
		...
		<tr>
			...
			<td><input type="radio" name="gender" value="0">男
				...
				<input type="radio" name="gender" value="2" checked>なし
			</td>
		</tr>
		<tr>
			<td>
				<select class="dropdown" name="authority">		
					...
					<option value="2" selected>なし</option>
				</select>
			</td>
			<td>
	...
	if(isset($_POST['family_name'])){
		...
		/*	↓全部の値が空でなおかつ性別＆権限が２（なし）の時＝そのまま検索ボタン押下*/
			if(empty($_POST['family_name']) and empty($_POST['first_name']) and empty($_POST['family_name_kana']) and empty($_POST['first_name_kana']) and empty($_POST['mail']) and $_POST['gender'] =="2" and $_POST['authority'] == "2"){
			...
			}elseif(isset($_POST['family_name']) or isset($_POST['first_name']) or isset($_POST['family_name_kana']) or isset($_POST['first_name_kana']) or isset($_POST['mail']) or isset($_POST['gender']) or isset($_POST['authority'])){
				echo "9";
				...
				if(isset($_POST['first_name'])){
					$sql = "where family_name LIKE '%$first_name%'";
				}
				if(isset($_POST['family_name_kana'])){
					$sql = "where family_name LIKE '%$family_name_kana%'";
				}
				if(isset($_POST['first_name_kana'])){
					$sql = "where family_name LIKE '%$first_name_kana%'";
				}
				if(isset($_POST['mail'])){
					$sql = "where family_name LIKE '%$mail%'";
				}
	...
	?>
			...
			<?php if(isset($_POST['family_name'])){foreach($rows as $row){
	?>
	<tr>
